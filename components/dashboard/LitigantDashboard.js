        ...
        STATUS_STEPS.map((step, i) => (
          <div key={step} className="flex items-center gap-1">
            <div className={`flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-semibold transition-colors ${
              ...
              i === currentIdx ? 'bg-amber-100 text-amber-700 ring-1 ring-amber-300' :
            ...
            {i < STATUS_STEPS.length - 1 && (
              <div className={`h-0.5 w-3 rounded ${i < currentIdx ? 'bg-green-300' : 'bg-stone-200'}`} />
            )}
          </div>
        ))
      )}
...
function CaseCard({ c }) {
  ...
  return (
    <Link href={`/dashboard/${c.uid}`}>
      <a className="block bg-stone-50 rounded-2xl border border-stone-200 shadow-sm hover:shadow-md hover:border-amber-200 transition-all p-5 group">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div>
            <p className="text-xs font-mono text-slate-400">{c.case_number}</p>
            <h3 className="text-base font-semibold text-slate-800 mt-0.5 group-hover:text-amber-700 transition-colors">{c.case_type}</h3>
          </div>
        ...
        {nextHearing && (
          <div className={`mt-3 flex items-center gap-2 rounded-xl p-3 ${daysUntil <= 1 ? 'bg-red-50' : daysUntil <= 3 ? 'bg-amber-50' : 'bg-stone-100'}`}>
            <svg className={`w-4 h-4 flex-shrink-0 ${daysUntil <= 1 ? 'text-red-500' : daysUntil <= 3 ? 'text-amber-500' : 'text-stone-500'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            <div>
              <p className={`text-xs font-semibold ${daysUntil <= 1 ? 'text-red-700' : daysUntil <= 3 ? 'text-amber-700' : 'text-stone-700'}`}>
...
function NextHearingBanner({ c }) {
  ...
  return (
    <Link href={`/dashboard/${c.uid}`}>
      <a className="block rounded-2xl bg-[linear-gradient(135deg,#0f172a_0%,#111827_55%,#1c1917_100%)] p-6 transition-colors hover:brightness-110">
        <p className="text-stone-400 text-xs uppercase tracking-widest mb-2">Your Next Hearing</p>
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-stone-50 text-xl font-bold">{c.case_number}</h2>
            <p className="text-stone-300 text-sm mt-1">{c.case_type}</p>
            <p className="text-stone-400 text-xs mt-1">{c.courtroom || 'Courtroom TBD'} · {c.assigned_magistrate || 'Magistrate TBD'}</p>
          </div>
          <div className="text-right flex-shrink-0">
            <p className="text-amber-300 text-3xl font-bold">{daysUntil === 0 ? 'Today' : `${daysUntil}d`}</p>
            <p className="text-stone-400 text-xs mt-1">{c.hearing_date}{c.hearing_time ? ` · ${c.hearing_time}` : ''}</p>
          </div>
        </div>
      </a>
    </Link>
    ...
    <div className="min-h-screen bg-stone-50">
      ...
      <div className="border-b border-stone-200 bg-stone-50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
          <p className="text-stone-500 text-xs uppercase tracking-widest font-medium mb-1">
            {new Date().toLocaleDateString('en-UG', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
          </p>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900">
            Welcome, <span className="text-amber-700">{userName.split(' ')[0]}</span>
          </h1>
          <p className="text-stone-500 text-sm mt-1">Litigant · Lwengo Grade I Magistrate&apos;s Court</p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="rounded-2xl border border-stone-200 bg-stone-50 p-4 text-center">
            ...
            <p className="text-xs text-stone-500 font-medium mt-1">Total Cases</p>
          </div>
          <div className="rounded-2xl border border-stone-200 bg-stone-50 p-4 text-center">
            <p className="text-3xl font-bold text-amber-700">{activeCases.length}</p>
            <p className="text-xs text-stone-500 font-medium mt-1">Active</p>
          </div>
          <div className="rounded-2xl border border-stone-200 bg-stone-50 p-4 text-center">
            <p className="text-3xl font-bold text-green-600">{concludedCases.length}</p>
            <p className="text-xs text-stone-500 font-medium mt-1">Concluded</p>
          </div>
        </div>
        ...
        <div className="bg-stone-100 border border-stone-200 rounded-2xl p-5">
          <p className="text-sm text-stone-700 font-semibold mb-1">Need help?</p>
          <p className="text-xs text-stone-600">
            Contact Lwengo Grade I Magistrate&apos;s Court Registry for case enquiries.
            Visit the court or check your <Link href="/dashboard/notifications"><a className="underline font-medium text-amber-700">notifications</a></Link> for updates.
          </p>
        </div>
      </div>
    </div>
